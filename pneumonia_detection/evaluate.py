...
#Evaluate DenseNet:
python -m pneumonia_detection.evaluate --mode densenet --plot

Compare Models (CNN, ResNet, DenseNet):
...
# DenseNet wrapper (added for DenseNet model)
try:
    from pneumonia_detection.densenet.densenet import (
        DenseNet121Binary,
    )  # DenseNet model
...
try:
    from pneumonia_detection.dataset import (
        get_dataloaders_resnet_aug as _resnet_eval_loaders,
    )
...
# DenseNet eval loader (RGB/ImageNet pipeline)
try:
    from pneumonia_detection.dataset import (
        get_dataloaders_densenet_aug as _densenet_eval_loaders,
    )

    _HAS_DENSENET_EVAL = True
except Exception:
    _HAS_DENSENET_EVAL = False

# CNN eval loader (grayscale pipeline)
try:
    from pneumonia_detection.dataset import (
        get_dataloaders_cnn_no_aug as _cnn_eval_loaders,
    )
...
def _load_test_loader(backbone: str):
    ...
    if backbone == "resnet":
        if not _HAS_RESNET_EVAL:
            print(
                "[WARN] ResNet eval loader not found. Falling back to DenseNet eval loader."
            )
        ...
        if not _HAS_DENSENET_EVAL:
            print(
                "[WARN] DenseNet eval loader not found. Falling back to ResNet eval loader."
            )
            return _load_test_loader("resnet")
        try:
...
def _load_model(
    model_path: Path, backbone: str, device: torch.device
) -> torch.nn.Module:
    if backbone == "resnet":
        if not _HAS_RESNET or ResNet18Binary is None:
            raise RuntimeError(
                "ResNet18Binary not available. Did you create pneumonia_detection/resnet.py?"
            )
        model = ResNet18Binary(pretrained=False).to(
            device
        )  # pretrained flag not needed for eval
    elif backbone == "densenet":
        if not _HAS_DENSENET or DenseNet121Binary is None:
            raise RuntimeError(
                "DenseNet121Binary not available. Did you create pneumonia_detection/densenet/densenet.py?"
            )
...
def _evaluate_one(
    plan: EvalPlan, device: torch.device, plot_cm: bool
) -> Dict[str, Any]:
    ...
    print(report_txt)
    print(
        f"Overall accuracy: {acc:.4f} | Weighted F1: {f1w:.4f} | MCC: {mcc:.4f} | Balanced Acc: {bacc:.4f}"
    )
    ...
    if plot_cm and _HAS_SNS:
        ...
        plt.title(f"Confusion Matrix — {plan.title}")
        plt.tight_layout()
        plt.savefig(f"confusion_matrix_{plan.backbone}.png")
        plt.close()
...
def _print_comparison(results: List[Dict[str, Any]]) -> None:
    ...
    print("\n================ Comparison (Test) ================")
    print(
        f"{'Model':<24} {'Backbone':<8} {'Acc':>7} {'F1w':>8} {'MCC':>8} {'BalAcc':>8} "
        f"{'NORMAL R':>10} {'PNEUMONIA R':>10}"
    )
    print("-" * 100)
    for m in results:
        rep = m["report"]
        ...
        print(
            f"{m['title']:<24} {m['backbone']:<8} {m['accuracy']:>7.4f} {m['f1_weighted']:>8.4f} "
...
def _plans_from_mode(mode: str, threshold: float) -> List[EvalPlan]:
    ...
    if mode == "aug":
        plans.append(
            EvalPlan(
                "CNN (with aug)", MODEL_DIR / "best_cnn_model_aug.pth", "cnn", threshold
            )
        )
    elif mode == "resnet":
        plans.append(
            EvalPlan(
                "ResNet (with aug)",
                MODEL_DIR / "best_resnet_model_aug.pth",
                "resnet",
                threshold,
            )
        )
        ...
        plans.append(
            EvalPlan(
                "DenseNet (with aug)",
                MODEL_DIR / "best_densenet_model_aug.pth",
                "densenet",
                threshold,
            )
        )
...
def main():
    ...
    ap.add_argument(
        "--plot", action="store_true", help="Show confusion matrix heatmap(s)."
    )
    args = ap.parse_args()
